...
{
    int i, j;
    int nGameResult = GAME_CONTINUE;
    int nRedSenseiFound = 0;
    int nBlueSenseiFound = 0;
    
    /* Check for sensei base occupation first */
    if (pGame->arrBoard[0][2] == BLUE_SENSEI) {
        nGameResult = BLUE_WINS;
    } else if (pGame->arrBoard[4][2] == RED_SENSEI) {
        nGameResult = RED_WINS;
    } else {
        /* Check if any sensei was captured */
        for (i = 0; i < BOARD_SIZE && nGameResult == GAME_CONTINUE; i++) {
            for (j = 0; j < BOARD_SIZE && nGameResult == GAME_CONTINUE; j++) {
...
int hasValidMoves(gameStateType *pGame, int nPlayer)
{
    ...
    /* Check each piece */
    for (i = 0; i < BOARD_SIZE && nHasValidMove == 0; i++) {
        for (j = 0; j < BOARD_SIZE && nHasValidMove == 0; j++) {
            ...
            if ((nPlayer == BLUE_PLAYER && (nPiece == BLUE_STUDENT || nPiece == BLUE_SENSEI)) ||
                ...
                /* Check each card */
                for (k = 0; k < PLAYER_CARDS && nHasValidMove == 0; k++) {
                    pCard = &pGame->arrPlayerCards[nPlayerIndex][k];
                    
                    /* Check each possible move */
                    for (l = 0; l < pCard->nMoveCount && nHasValidMove == 0; l++) {
