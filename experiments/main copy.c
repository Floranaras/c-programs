...
/* Temple positions */
#define RED_TEMPLE_ROW 0
#define RED_TEMPLE_COL 2
#define BLUE_TEMPLE_ROW 4
#define BLUE_TEMPLE_COL 2
...
void checkTempleWin(gameDatabaseType* pDb, int nRow, int nCol) 
{
    if (pDb->arrBoard[nRow][nCol] == RED_MASTER) {
        ...
        if (nRow == BLUE_TEMPLE_ROW && nCol == BLUE_TEMPLE_COL) {
            pDb->nWinner = RED;
    ...
    if (pDb->arrBoard[nRow][nCol] == BLUE_MASTER) {
        ...
        if (nRow == RED_TEMPLE_ROW && nCol == RED_TEMPLE_COL) {
            pDb->nWinner = BLUE;
            pDb->nWinCondition = WIN_TEMPLE;
            pDb->bGameOver = TRUE;
        }
...
void validateCardMove(gameDatabaseType* pDb, int nFromRow, int nFromCol, int nToRow, int nToCol, int nCardIndex) 
{
    ...
    if (pDb->bTempValidMove == TRUE) {
        ...
        bMoveFound = FALSE;
        i = 0;
        while (i < pCard->nNumMoves && bMoveFound == FALSE) {
            nCardDx = pCard->arrMoves[i].nDx;
            nCardDy = pCard->arrMoves[i].nDy;
            
            /* Only BLUE player needs moves flipped - they see board from opposite perspective */
            if (pDb->nCurrentPlayer == BLUE) {
                nCardDx = -nCardDx;
                nCardDy = -nCardDy;
            }
            /* RED player uses moves as-is from card definition */
...
void printBoardRow(gameDatabaseType* pDb, int nRow) 
{
    ...
    while (j < BOARD_SIZE) {
        ...
        if (pDb->arrBoard[nRow][j] == RED_PAWN) {
            ...
            if (nRow == RED_TEMPLE_ROW && j == RED_TEMPLE_COL) {
                cPiece = '*'; /* Red temple (Blue's target) */
            } else if (nRow == BLUE_TEMPLE_ROW && j == BLUE_TEMPLE_COL) {
...
void printGameInfo(gameDatabaseType* pDb) 
{
    ...
    printf("  RED wins by reaching (%d,%d) | BLUE wins by reaching (%d,%d)\n", 
           BLUE_TEMPLE_ROW, BLUE_TEMPLE_COL, RED_TEMPLE_ROW, RED_TEMPLE_COL);
}
...
void displayGameResult(gameDatabaseType* pDb) 
{
    ...
    if (pDb->nWinCondition == WIN_CAPTURE) {
        ...
        if (pDb->nWinner == RED) {
            printf("Red master reached Blue's temple at (%d,%d)!\n", BLUE_TEMPLE_ROW, BLUE_TEMPLE_COL);
        } else {
            printf("Blue master reached Red's temple at (%d,%d)!\n", RED_TEMPLE_ROW, RED_TEMPLE_COL);
        }
    }
...
void checkForValidMoves(gameDatabaseType* pDb) 
{
    ...
    bFoundValidMove = FALSE;
    
    /* Check both cards */
    nCardIndex = 0;
    while (nCardIndex < CARDS_PER_PLAYER && bFoundValidMove == FALSE) {
        pCard = &pDb->arrPlayerCards[pDb->nCurrentPlayer][nCardIndex];
        
        /* Check all pieces on board */
        nPieceRow = 0;
        while (nPieceRow < BOARD_SIZE && bFoundValidMove == FALSE) {
            nPieceCol = 0;
            while (nPieceCol < BOARD_SIZE && bFoundValidMove == FALSE) {
                ...
                if ((pDb->nCurrentPlayer == RED && (pDb->arrBoard[nPieceRow][nPieceCol] == RED_PAWN || pDb->arrBoard[nPieceRow][nPieceCol] == RED_MASTER)) ||
                    ...
                    nMoveIndex = 0;
                    while (nMoveIndex < pCard->nNumMoves && bFoundValidMove == FALSE) {
                        nCardDx = pCard->arrMoves[nMoveIndex].nDx;
                        nCardDy = pCard->arrMoves[nMoveIndex].nDy;
                        
                        /* Only BLUE player needs coordinate adjustment */
                        if (pDb->nCurrentPlayer == BLUE) {
                            nCardDx = -nCardDx;
                            nCardDy = -nCardDy;
                        }
    ...
    pDb->bHasValidMoves = bFoundValidMove;
}
